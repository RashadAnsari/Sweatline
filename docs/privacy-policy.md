...
- Your settings: weight unit (kg or lb) and theme choice.
...
clipboard as text, and an "Import" action that reads it back. What you do with
the copied text is entirely up to you. Sweatline does not upload it.
...
feedback for the rest timer. It requests no runtime permissions, no location,
no camera, no contacts, no health data, and no internet access.
